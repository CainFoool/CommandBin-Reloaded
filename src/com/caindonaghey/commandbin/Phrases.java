...
public class Phrases {
	...
	public static String get(String string) {
		...
		if(CommandBin.language == "dutch") {
			...
			if(string == "spy-on") return prefix + "You are now spying on player's messages.";
			if(string == "spy-off") return prefix + "You are no longer spying on player's messages.";
			if(string == "vote-cancelled") return badPrefix + ChatColor.YELLOW + "De stemming is geannuleerd!";
			if(string == "vote-not-running") return badPrefix + "Er is geen stemming bezig.";
			if(string == "vote-started-1") return prefix + ChatColor.YELLOW + "Er is een nieuwe stemming gestart!";
			if(string == "vote-started-2") return prefix + ChatColor.YELLOW + "\" {VOTE} \"";
			if(string == "vote-started-3") return prefix + ChatColor.YELLOW + "/voteyes of /voteno";
			if(string == "already-voted") return badPrefix + "Je hebt al gestemd.";
			if(string == "vote-bro") return prefix + ChatColor.YELLOW + "{PLAYER} heeft gestemd {VOTE} ({1}/{2})";
			if(string == "thanks-voting") return prefix + "Bedankt om te stemmen!";
			if(string == "vote-yes") return prefix + ChatColor.YELLOW + "Ja is gewonnen!";
			if(string == "vote-no") return prefix + ChatColor.YELLOW + "Nee is gewonnen!";
			if(string == "vote-elegible") return prefix + "Je bent niet bevoegd om te stemmen.";
			if(string == "vote-running") return prefix + "Er is al een stemming bezig!";
			if(string == "vote-tied") return prefix + ChatColor.YELLOW + "The vote tied!";
			if(string == "carpet-on") return prefix + "Carpet Mode now enabled.";
			if(string == "carpet-off") return prefix + "Carpet Mode now disabled.";
		}
