	...
	public boolean onCommand(CommandSender s, Command c, String l, String [] args) {
		if(l.equalsIgnoreCase("feed")) {
			if(!(s instanceof Player)) {
				...
				System.out.println("[CommandBin] " + Phrases.get("fed-other"));
				return true;
			}
			...
			if(args.length == 0) {
				if(player.hasPermission("CommandBin.feed.self")) {
					player.setFoodLevel(20);
					player.sendMessage(ChatColor.GREEN + "[CommandBin] " + Phrases.get("fed-self"));
					return true;
				}
			...
			if(args.length == 1) {
				if(player.hasPermission("CommandBin.feed.others")) {
					...
					player.sendMessage(ChatColor.GREEN + "[CommandBin] " + Phrases.get("fed-other"));
					return true;
				}
				player.sendMessage(ChatColor.RED + "[CommandBin] " + Phrases.get("no-permission"));
			return true;
